...
from modules.folder_ops import FolderOps, _uri_to_path
from modules.ide_ops import IdeOps, _is_file_openable_in_ide
from modules.image_convert_ops import ImageOps, is_image_file
from modules.image_compress_ops import ImageCompressOps, is_image_file as is_compress_image_file
...
class NautilusFileMenu(GObject.Object, Nautilus.MenuProvider):
    def _window_added(self, application, window):
        for key, shortcut_str in self.config.get("shortcuts", {}).items():
            if shortcut_str:
    ...
    def __init__(self):
        ...
        self.image_ops = ImageOps(self.config)
    ...
    def _shortcuts_handler(self, window, key):
        ...
        action_function = {
            "path": self.copy_ops.copy_paths,
            "uri": self.copy_ops.copy_uris,
            "name": self.copy_ops.copy_names,
            "content": self.copy_ops.copy_content,
        }.get(action)
    ...
    def _create_menu_items(self, files, group):
        ...
        if config_items.get("content", True):
            allow_copy_content = ["application/x-shellscript", "application/json"]
            filtered = [
                f for f in files
                if f.get_mime_type() in allow_copy_content
                or f.get_mime_type().startswith("text/")
            ]
            if filtered:
                item = Nautilus.MenuItem(
                ...
                item.connect("activate", self.copy_ops.copy_content, filtered)
                items.append(item)
        ...
        # --- IDE operations (skip if any selected file is media/archive) ---
        if config_items.get("open_ide", True):
            ide_files = [f for f in files if _is_file_openable_in_ide(f)]
            other_ides = self.ide_ops.get_other_ides()
            jb_ides = self.ide_ops.get_jetbrains_ides()
            has_any_ide = other_ides or jb_ides

            if has_any_ide and ide_files:
                submenu = Nautilus.Menu()

                # Non-JetBrains IDEs: direct entries
        ...
        # --- Image conversion ---
        if config_items.get("image_convert", True):
            ...
            if image_files:
                ...
                if formats:
                    ...
                    for fmt in formats:
                        ...
                        sub_item.connect(
                            "activate",
                            lambda m, f, fmt=fmt: self.image_ops.convert_image(m, f, fmt),
                            image_files,
                        )
                        submenu.append_item(sub_item)
        ...
        # --- Image compression ---
        if config_items.get("image_compress", True):
            compress_files = [f for f in files if is_compress_image_file(f)]
            if compress_files:
                submenu = Nautilus.Menu()
                ...
                size_item.connect(
                    "activate",
                    lambda m, f: self.image_compress_ops.resize_by_dimensions(m, f),
                    compress_files,
                )
                ...
                percent_item.connect(
                    "activate",
                    lambda m, f: self.image_compress_ops.resize_by_percent(m, f),
                    compress_files,
                )
        ...
        # --- Checksum (only for files, not folders) ---
        checksum_files = [f for f in files if not os.path.isdir(_uri_to_path(f))]
        if config_items.get("checksum", True) and checksum_files:
